...
/**
 * 支持获取父类属性，支持父类JPA column注解
 * Created by wangzhongfu on 2015/12/25.
 */
public class BeanProcessorSupportJpaAnnotation {
     ...
     */
    public BeanProcessorSupportJpaAnnotation() {
     ...
     * @since 1.5
     */
    public BeanProcessorSupportJpaAnnotation(Map<String, String> columnToPropertyOverrides) {
        super();
    ...
    protected int[] mapColumnsToProperties(ResultSetMetaData rsmd,
        ...
        for (int col = 1; col <= cols; col++) {
            ...
            String propertyName = columnToPropertyOverrides.get(columnName.toLowerCase());
        ...
        for(Field field : fields) {
            if(field.isAnnotationPresent(ColAlias.class)) {
                ...
                if(!StringUtils.isEmpty(colAlias.value())) {
                    columnToPropertyOverrides.put(colAlias.value().toLowerCase(), field.getName());
                }
            }
        }*/
        Class tempType = type;
        int depth = 1;
        while(!tempType.equals(Object.class) && !primitiveDefaults.containsKey(tempType) && depth <= maxDepth){
            Field[] fields = tempType.getDeclaredFields();
            for(Field field : fields) {
                if(field.isAnnotationPresent(Column.class)){
                    Column column = field.getAnnotation(Column.class);
                    if(!StringUtils.isEmpty(column.name())) {
                        columnToPropertyOverrides.put(column.name().toLowerCase(), field.getName());
                    }
                }
                if(field.isAnnotationPresent(ColAlias.class)) {
                    ColAlias colAlias = field.getAnnotation(ColAlias.class);
