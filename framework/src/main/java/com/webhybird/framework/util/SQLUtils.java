...
public class SQLUtils {
	  ...
	  public static String trimFrom(String sql) {
	    ...
	    return matcher.find() ? sql.substring(matcher.start()) : ""; //后面的字�?
	  }
	  
	  /**
	   * 判断SQL语句中是否包含distinct关键�?
	   * @param sql
	   * @return
	   */
	  ...
	  public static String replaceOrderBy(String sql) {
	    ...
	    Pattern pattern = Pattern.compile(patternString);
	    Matcher matcher = pattern.matcher(sql);
	    String wantStr = matcher.replaceAll(" ORDER BY "); //后面的字�?
	    return wantStr;
	  }
